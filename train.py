...
minority_up = resample(minority, replace=True, n_samples=TARGET, random_state=42)

df_bal = pd.concat([majority_down, minority_up]).sample(frac=1, random_state=42)

X_bal = df_bal[feature_cols].values
y_bal = df_bal['label'].values

print(f"   After balancing — Safe: {(y_bal==0).sum()} | Risk: {(y_bal==1).sum()}")
